...
# 202003890_4.asm -- an implementation of a 4x4 Sudoku Solver
...
.macro	get_elem(%elem, %index, %addr, %base)		# get a specific element from an array in the grid
		addu	%addr, %index, %base		# %addr = address of the element
		lw	%elem, (%addr)			# %elem = element
.end_macro

.macro	box_coord(%coord, %box)			# finds the coords of the box the given square is part of
		div	%box, %coord, 8			# %box = %coord // 8
		li	$t0, 8				# $t0 = 8
		mult	%box, $t0			# %box * 8
		mflo	%box				# %box = %box * 8
.end_macro
...
.text
main:
		li	$a0, 16				# allocates memory for an array
		do_syscall(9)				# this array will store the entire grid
		move	grid, $v0			# grid = pointer to the first element in the array
		
		read_row(0)				# read first row of the grid
		read_row(4)				# read second row of the grid
		read_row(8)				# read third row of the grid
		read_row(12)				# read fourth row of the grid
...
check:		#####preamble######			# arguments: $a0 = row, $a1 = col, $a2 = n, $a3 = grid
		...
		li	$t1, 0				# $t1 = 0
check_row:	
		bgt	$t1, $t0, cr_exit		# if $t1 > 12, exit loop
...
check_column:
		bgt	$t1, $t0, cc_exit		# if $t1 > 12, exit loop
...
cc_exit:	box_coord($a0, row0)			# find the row of the 2x2 box the given square belongs to
		box_coord($a1, col0)			# find the col of the 2x2 box the given square belongs to

		li	$t0, 4				# $t0 = 4
		li	$t1, 0				# $t0 = 0
		li	$t2, 0				# $t2 = 0
check_box:
		bgt	$t1, $t0, check_ret_t		# if $t1 > 4, return true
check_box2:
		bgt	$t2, $t0, ext_cs2		# if $t2 > 4, exit check_box2
...
solve_row:
		bgt	$s1, 12, solve_ret_t		# if $s1 > 12, return true to prevent backtracking
solve_col:
		bgt	$s2, 12, ext_solve_col		# if $s2 > 12, exit solve_col
...
check_n:	li	$s4, 1				# n = 1
check_n_loop:
		bgt	$s4, 4, solve_ret_f		# if n > 4, return false to backtrack
		move	$a0, $s1			# $a0 = row
		move	$a1, $s2			# $a1 = col
		move	$a2, $s4			# $a2 = n
...
place_n:
		...
		bgt	$a2, 12, next_row		# if $a2 > 12, j next_row
...
loop_row:
		bgt	$s1, 12, print_ret		# if $s1 > 12, return
loop_col:
		bgt	$s2, 12, ext_loop_col		# if $s2 > 12, exit loop_col
		get_elem($t1, $s1, $t0, grid)		# $t1 = grid[row]
